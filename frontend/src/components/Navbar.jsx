...
const Navbar = () => {
    ...
    useEffect(() => {
        // Listen for advanced filter toggle from Home
        const handleToggle = (e) => setIsHidden(e.detail);
    ...
    return (
        <>
            <nav className="bg-white/80 backdrop-blur-md border-b border-slate-100 sticky top-0 z-50 h-20">
            ...
            <AnimatePresence>
                {isOpen && (
                    <div className="md:hidden">
                        ...
                        >
                            <div className="flex justify-between items-center p-6 border-b border-slate-100 h-20">
                            ...
                            <div className="p-6 space-y-6 overflow-y-auto custom-scrollbar flex-1">
                                {user ? (
                                    <>
                                        <div className="p-4 bg-slate-50 rounded-2xl flex items-center space-x-4 mb-4">
                                            <CompressedImage
                    ...
                    {isOpen && user && (
                        <>
                            ...
                            <Motion.div
                                ...
                                className="fixed top-0 right-0 h-full w-80 bg-white shadow-2xl z-[70] border-l border-slate-100 overflow-y-auto custom-scrollbar"
                            >
                                <div className="flex flex-col min-h-full p-6">
                                    ...
                                    <div className="flex-1 space-y-2">
                                        <Link to="/profile" className="drawer-nav-link" onClick={() => setIsOpen(false)}>
                                            <Settings className="w-5 h-5" />
                                            <span>{t('navbar.profile')}</span>
                                        </Link>
                                        <Link to="/my-listings" className="drawer-nav-link" onClick={() => setIsOpen(false)}>
                                            <LayoutDashboard className="w-5 h-5" />
                                            <span>{t('navbar.my_listings')}</span>
                                        </Link>
                                        <Link to="/favorites" className="drawer-nav-link" onClick={() => setIsOpen(false)}>
                                            <Heart className="w-5 h-5" />
                                            <span>{t('navbar.favorites') || 'Favorites'}</span>
                                        </Link>
                                        <Link to="/create-announcement" className="drawer-nav-link primary" onClick={() => setIsOpen(false)}>
